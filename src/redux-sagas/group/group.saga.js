...
export function* setMessageSeen({ payload }) {
  try {
    const res = yield call(
      PrivateApiRoute,
      `group/${payload}/seen`,
    ...
    console.log(res);
    yield put(setMessageSeenSuccess());
  } catch (error) {
    console.log(error);
    yield put(setMessageSeenFailure());
  }
}

export function* OnSetSeenStart() {
  yield takeLatest(groupActionTypes.SET_MESSAGE_SEEN_START, setMessageSeen);
}
